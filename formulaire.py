...
class Formulaire:
    def __init__(self, root):
        ...
        btn = Button(frame1, text="créer", cursor="hand2",command=self.creer, font=("arial", 15,), bg="cyan", fg="black")
        btn.place(x=250, y=430)
        btn2 = Button(frame1, text="connexion", cursor="hand2", font=("arial", 15,), bg="cyan", fg="black")
    ...
    def creer(self):
        if self.ecri_prenom.get()=="" or self.ecri_email.get()=="" or self.ecri_nom.get()=="" or self.ecri_mdp.get()=="" or self.ecri_confmdp.get()=="":
            messagebox.showerror("Erreur","à quoi tu joues ? tu veux que jte ban ip fdp ? Allez hop hop faut tout remplir sans se tromper", parent=self.root)
        elif  self.ecri_prenom.get()==" " or self.ecri_email.get()=="" or self.ecri_nom.get()=="" or self.ecri_mdp.get()=="" or self.ecri_confmdp.get()=="":
            messagebox.showerror("Erreur","à quoi tu joues ? tu veux que jte ban ip fdp ? Allez hop hop faut tout remplir sans se tromper", parent=self.root)
