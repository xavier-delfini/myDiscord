...
class Form:
    def __init__(self):
        ...
        btn = Button(frame1, text="créer", cursor="hand2", command=self.create_new_user, font=("arial", 15,), bg="cyan", fg="black")
        btn.place(x=250, y=430)
        btn2 = Button(frame1, text="connexion", cursor="hand2", command=self.create_login_window, font=("arial", 15,), bg="cyan", fg="black")
    ...
    def create_new_user(self):
        if self.ecri_prenom.get()=="" or self.ecri_email.get()=="" or self.ecri_nom.get()=="" or self.ecri_mdp.get()=="" or self.ecri_confmdp.get()=="":
            ...
            messagebox.showerror("Erreur","Pas d'espace ou autre caracter pour les nom ou prénom juste -", parent=self.root)
